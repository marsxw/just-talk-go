...
- 自动复制到剪贴板，支持自动上屏。
- Wayland / X11 / macOS 顶层录音状态胶囊提示，录音时实时显示识别文字。
- TUI 配置界面，支持热键、模式、自动上屏、停止延迟、最长录音时长、热词等配置。
...
命令行控制录音（需要 just-talk 已在运行）：

```bash
just-talk --toggle-recording   # 切换录音（toggle 模式等同按一次热键）
just-talk --start-recording    # 开始录音
just-talk --stop-recording     # 停止录音
just-talk --recording-status   # 查看当前录音状态
```
